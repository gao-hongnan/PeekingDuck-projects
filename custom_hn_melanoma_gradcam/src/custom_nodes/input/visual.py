    ...
    **Technotes:**
    ...
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.file_end and self.has_multiple_inputs:
            ...
            self.logger.debug(
                f"#frames={self.frame_counter}, done={self.progress}%"
            )
            self._get_next_file()
            outputs = self._get_next_frame()
    ...
    def _open_input(self, input_source: Any) -> None:
        ...
        if self.threading:
            self.videocap = VideoThread(
                self.source, self.mirror_image, self.buffering
            )
        else:
    ...
    def _show_progress(self) -> None:
        ...
        if self.frame_counter % self.frames_log_freq == 0 and self.videocap:
            ...
            self.logger.info(
                f"Frames Processed: {self.frame_counter}{buffer_info}"
            )
        if self.total_frame_count > 0:
            self.progress = round(
                (self.frame_counter / self.total_frame_count) * 100
            )
